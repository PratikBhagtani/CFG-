...
public class CfgGraph {
	...
	private List<ConEdgs> edges;		// Edge list
	...
	public void AddEdge(ConEdgs _edge) {
		edges.add(_edge);
	}
	...
	public List<ConEdgs> GetEdgeStartFrom(Node _node) {
		List<ConEdgs> chosenEdges = new LinkedList<ConEdgs>();
		Iterator<ConEdgs> iterator = edges.iterator();
		ConEdgs edge;
		while(iterator.hasNext()) {
			edge = iterator.next();
...
//		System.out.println("dnhsjuskjnsk");
	}
	...
	private void getNodes(){
		...
		if (printDebug){
			...
			for (ConEdgs e: edges) System.out.println("("+e.GetStart()+","+e.GetEnd()+")");
		}
		...
		for (int i=0; i<nodes.size(); i++){
			...
			if (outEdges.size() > 0){ //if false, this is the last node
				...
				for (int idx: outEdges){
					edges.set(idx, new ConEdgs(nodes.get(i).GetNodeNumber(), edges.get(idx).GetEnd()));
	...
	private void numberNodes(){
		...
		List<ConEdgs> oldedges = new ArrayList<ConEdgs>();
		for (ConEdgs e: edges) oldedges.add(new ConEdgs(e.GetStart(), e.GetEnd()));
		edges.clear();
	...
	private void fixNumbering(){
		...
		for (int i=0; i<nodes.size(); i++){
			Node n = nodes.get(i);
			for (int j=0; j<edges.size(); j++){
				if (edges.get(j).GetStart() == n.GetNodeNumber()) edges.set(j, new ConEdgs(i, edges.get(j).GetEnd()));
				if (edges.get(j).GetEnd() == n.GetNodeNumber()) edges.set(j, new ConEdgs(edges.get(j).GetStart(), i));
			}
			n.SetNodeNumber(i);
			nodes.set(i, n);
		}
	...
	private String generateDOT(){
		...
		System.out.println("Number of nodes are:"+ nodes.size());
		for (Node n: nodes){
...
//				System.out.println("fggyhjki");
	...
	private void addEdge(int startidx, int endidx){
		...
		edges.add(new ConEdgs(startidx,endidx));
